...
echo ""
echo "Or use the base template:"
echo "  cp snmpd-base-config /etc/config/snmpd"
echo "  $SCRIPT_DIR/snmpd-config-generator.sh >> /etc/config/snmpd"
...
if [ -z "$answer" ] || [ "$answer" = "y" ]; then
    ...
    # 2. Overwrite with base config
    cp "$SCRIPT_DIR/snmpd-base-config" /etc/config/snmpd
    
    # 3. Append generated config
